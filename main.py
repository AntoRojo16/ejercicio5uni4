...
# Instalar pillow para interfaz
from PIL import ImageTk, Image
import requests
import json
import io

#lista de géneros de la API
response = requests.get(
    "https://api.themoviedb.org/3/genre/movie/list?api_key=6bd5e7fdf1de00d2f53c7a10a37ae32f&language=es")
genres_data = json.loads(response.text)["genres"]
genres_dict = {genre["id"]: genre["name"] for genre in genres_data}

#lista de las películas de la API
response = requests.get(
    "https://api.themoviedb.org/3/discover/movie?api_key=6bd5e7fdf1de00d2f53c7a10a37ae32f&language=es")
movies_data = json.loads(response.text)["results"]
movies_list = [(movie["title"], movie["overview"], movie["original_language"], movie["release_date"],
                [genres_dict[genre_id] for genre_id in movie["genre_ids"]], movie["poster_path"]) for movie in
               movies_data]
    ...
    if selection:
       ...
       #imagen de la película
        ...
        if response.status_code == 200:
            ...
            image = image.resize((250, 375))  # Ajustar el tamaño
